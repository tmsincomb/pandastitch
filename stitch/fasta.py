...
class Dna:
    ...
    def __init__(self, header, sequence, quality: bool = False):
        ...
        if len(self.seq) != len(self.qual):
            raise IOError(f"Seq length and qual length do not agree: {self.header}")
    ...
    def __str__(self):
        ...
        if not self.qual:
            ...
            return ("@%s\n%s\n+%s\n%s") % (self.header, self.seq, self.header, self.qual)
