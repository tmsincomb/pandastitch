...
def stitch(*args, **kwargs):
    ...
    for i in p.imap(doStitch, izip(Fasta(seqsa), Fasta(seqsb))):
        ...
        if i.score > score:
            ...
            if prefix:
                print("%s" % i.record, file=outfile)

            if pretty:
                print(">%s (%.3f)" % (i.reca.header, i.score), file=sys.stdout)
                print(i.pretty, file=sys.stdout)

            if table:
                print(i.overlap, file=htable)
        else:
            reca, recb = i.originals

            if prefix:
                print(reca, file=dudsa)
                print(dudsb, recb, file=dudsa)
    ...
    try:
        print("Average overlap was %.2f\n" % (float(overlaps) / numcontigs), file=sys.stderr)
...
class Stitch:
    ...
    def find_overlaps(self):
        ...
        middle = []
        qual_middle = []
        for (i, qi), (j, qj) in zip(zip(a[best_index:], qa[best_index:]), zip(b[:-best_index], qb[:-best_index])):
            if i == j:
                middle.append(i)
        ...
        self.pretty = "1:%s\n2:%s\nC:%s\n" % (a + "-" * (best_index - 1), "-" * (best_index - 1) + b, newseq)
...
def get_args():
    ...
    parser.add_option("-i", "--first", dest="filea", help="first fastq file")

    parser.add_option("-j", "--second", dest="fileb", help="second fastq file")

    parser.add_option("-o", "--output", dest="prefix", help="output prefix (omit to print to stdout)")

    parser.add_option(
        "-t", "--threads", dest="threads", default=None, type=int, help="number of threads (default = all available)"
    )
    ...
    parser.add_option(
        "-s", "--score", dest="score", default=35, help="minimum percent identity (default = 25)", type=float
    )
...
def main():
    ...
    if not (options.filea and options.fileb):
        print("Usage: %s %s" % (parser.get_prog_name(), parser.usage), file=sys.stderr)
        sys.exit()

    # output everything to stdout if no output prefix is specified
    if not (options.prefix):
        print("Warning: no outputfile", file=sys.stderr)
